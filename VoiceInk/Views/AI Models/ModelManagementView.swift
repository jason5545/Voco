                ...
                VStack(spacing: 12) {
                    ...
                    if selectedFilter == .local {
                        HStack(spacing: 8) {
                            ...
                            InfoTip(
                                "Add a custom fine-tuned whisper model to use with Voco. Select the downloaded .bin file.",
    ...
    private var filteredModels: [any TranscriptionModel] {
        ...
        case .recommended:
            return whisperState.allAvailableModels.filter {
                let recommendedNames = ["ggml-base.en", "parakeet-tdt-0.6b-v2", "ggml-large-v3-turbo-q5_0", "whisper-large-v2-mlx-4bit", "whisper-large-v3-turbo"]
                return recommendedNames.contains($0.name)
            }.sorted { model1, model2 in
                let recommendedOrder = ["ggml-base.en", "parakeet-tdt-0.6b-v2", "ggml-large-v3-turbo-q5_0", "whisper-large-v2-mlx-4bit", "whisper-large-v3-turbo"]
        ...
        case .local:
            return whisperState.allAvailableModels.filter {
                if SystemArchitecture.isIntelMac && ($0.provider == .qwen3 || $0.provider == .whisperMLX) { return false }
                return $0.provider == .local || $0.provider == .nativeApple || $0.provider == .parakeet || $0.provider == .qwen3 || $0.provider == .whisperMLX
            }
        case .cloud:
