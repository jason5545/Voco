        ...
        .overlay(
            VStack {
                if let state = bannerState {
                    ...
                    case .retranscribeError(let msg):
                        if msg.isEmpty {
                            StatusBanner(message: "Retranscription failed", isError: true)
                        } else {
                            StatusBanner(message: LocalizedStringKey(msg), isError: true)
                        }
                    case .reEnhanceError(let msg):
                        if msg.isEmpty {
                            StatusBanner(message: "Re-enhancement failed", isError: true)
                        } else {
                            StatusBanner(message: LocalizedStringKey(msg), isError: true)
                        }
                    }
