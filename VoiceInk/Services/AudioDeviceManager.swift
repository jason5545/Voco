...
class AudioDeviceManager: ObservableObject {
    private let logger = Logger(subsystem: AppIdentifiers.subsystem, category: "AudioDeviceManager")
    ...
    func getCurrentDevice() -> AudioDeviceID {
        ...
        case .prioritized:
            ...
            for saved in sortedDevices {
                if let found = findAvailableDevice(uid: saved.id, modelUID: saved.modelUID) {
                    return found.id
                }
            }
            return findBestAvailableDevice() ?? 0
