...
class GroqTranscriptionService {
    private let logger = Logger(subsystem: AppIdentifiers.subsystem, category: "GroqService")
    ...
    private func makeTranscriptionRequest(audioURL: URL, model: any TranscriptionModel) async throws -> String {
        ...
        if !(200...299).contains(httpResponse.statusCode) {
            ...
            logger.error("Groq API request failed with status \(httpResponse.statusCode): \(errorMessage, privacy: .private)")
