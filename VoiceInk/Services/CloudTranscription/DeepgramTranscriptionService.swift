...
class DeepgramTranscriptionService {
    private let logger = Logger(subsystem: AppIdentifiers.subsystem, category: "DeepgramService")
    ...
    func transcribe(audioURL: URL, model: any TranscriptionModel) async throws -> String {
        ...
        if !(200...299).contains(httpResponse.statusCode) {
            ...
            logger.error("Deepgram API request failed with status \(httpResponse.statusCode): \(errorMessage, privacy: .private)")
