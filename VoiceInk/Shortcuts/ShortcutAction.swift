    ...
    var displayName: String {
        ...
        case .powerMode(let id):
            if let config = PowerModeManager.shared.getConfiguration(with: id) {
                return String(localized: "\(config.name) Power Mode")
            }
        ...
        case .miniRecorderPrompt(let index):
            return String(localized: "Select Prompt \(Self.displayNumber(forMiniRecorderIndex: index))")
        case .miniRecorderPowerMode(let index):
            return String(localized: "Select Power Mode \(Self.displayNumber(forMiniRecorderIndex: index))")
        }
    }
