...
enum ShortcutValidationError: Equatable {
    ...
    func notificationTitle(for shortcut: Shortcut) -> String {
        switch self {
        case .plainKeyRequiresModifier:
            return String(localized: "Shortcut not allowed: \(shortcut.displayString)")
        case .shiftTypingKeyRequiresAdditionalModifier:
            return String(localized: "Shortcut not allowed: \(shortcut.displayString)")
        case .reservedBySystem:
            return String(localized: "Shortcut reserved by macOS: \(shortcut.displayString)")
        case .alreadyUsedBy(let actionName):
            return String(localized: "Shortcut already used by \(actionName)")
        }
    }
}
