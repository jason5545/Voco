enum AIPrompts {
    ...
    - 程式相關：大宇→大語言（大語言模型）、成事→程式、專欄→專案、日劇→日誌、日子→日誌、日記→日誌（查看日誌語境）、不服→部署（部署應用程式、部署版本語境）、英雄黨→音訊檔、SX CODE→Xcode、GY→GUI（圖形介面）
    - 語音輸入：雨停→語音（語音辨識）
    ...
    標點符號禁止規則（違反任何一條就等於失敗）：
    - 絕對不能把詞語拆開插逗號：「問題」「時候」「軟體」「資源」「經驗」「修改」「按鍵」「方法」「角度」「套路」等都是完整詞，中間不能插逗號
    - 絕對不能在「的」「了」「是」「得」後面加逗號，除非後面是新子句的主語（有明確主詞切換）
    - 錯誤示範：「增強管線的，問題」「在餐廳的，時候」「足夠的，資源」「深度了，解」→ 這些全部是錯的
    - 正確示範：「增強管線的問題」「在餐廳的時候」「足夠的資源」「深度了解」→ 不要在這些地方插逗號
    ...
    static func conservativeRetryPrompt(uncertainWords: [String]) -> String {
        ...
        - 絕對不能在「的」「了」「是」「得」後面加逗號，除非後面是新子句的主語
        - 絕對不能把詞語拆開插逗號（如「問題」「時候」「軟體」中間不能有逗號）
        """
    }
    ...
    - 不要在「的」「了」「是」「得」後面加逗號，除非後面是新子句的主語（有明確主詞切換）
    - 逗號只加在子句邊界：主語切換處、連接詞（但是、所以、因為、而且）前後
    - 3-5 個字的短語不需要逗號，至少 8 個字以上的片段才考慮加逗號
    """
