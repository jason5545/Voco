...
## 這是個人專案——也是給非典型語音的一條可行路線（POC）

Voco 首先是**一個人的日用工具**：它圍繞唯一一把聲音打造、調校、把關——作者的聲音。作者有腦性麻痺（CP），現成的 ASR 對非典型語音幾乎棄械投降；這個 repo 存在的意義是證明有一條可行的路：

1. **收集自己的真實語料**，附上確認過的 transcript。
2. **用自己的聲音訓練 speaker adapter**（在開源 ASR 模型上做 LoRA）。
3. **用自己的 key rows 把關**——在你最難的真實句子上證明有效之前，不進日常使用。

同樣的路線適用於任何語調／構音受疾病影響的人：**腦性麻痺（CP）、漸凍人（ALS）、帕金森氏症、中風、多發性硬化症、肌肉萎縮症、小腦性共濟失調、創傷性腦傷、聽損相關構音差異**等——凡是構音障礙讓通用 ASR 失靈的地方。

**歡迎 fork——但請不要開箱即用。** 這裡提到的任何 adapter 或模型 artifact 都是拿作者的聲音訓練的；對任何其他人，它只會讓辨識**更差**，不會更好。可重用的是方法與管線，權重是個人的。**收集自己的資料、訓練自己的 adapter、再來用它。** 本專案為個人專案，永不做商用。
